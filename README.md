...
獅尾喇腿黑體基於[思源黑體](https://github.com/adobe-fonts/source-han-sans)的筆觸變喇叭口改造，更加簡明現代化的字體。支援简体中文、繁體中文、韓文與日文；可以免費商用，歡迎大家自由應用、自由優化、自由改作！

獅尾喇腿黑體的特色是：筆畫的起頭與收尾較寬，有喇叭造型。「喇叭口」在鉛字印刷時代有讓印刷字安定清晰的用途，目前電腦螢幕顯示與高解析度輸出的數位時代，喇叭口變成字型的風格的一部份。

「獅尾喇叭黑體」與「獅尾喇腿黑體」差別在有沒有拔腳，「拔腳」就是會有較少的筆畫，例如移除「口」字的左右下角會多出的短線條。有拔腳的字會較圓，但是會造成有些字因直接去掉腿後左右不平衡、左右不對襯的「懸空」問題。改使用「腿」系列字體就不會有這個問題。

![字體預覽](https://github.com/max32002/swei-bell-leg/raw/master/preview/welcome.png)

「獅尾喇腿黑體」設計上針對筆畫粗細支援7種字重，所以不論大標、小標、簡單的段落排版都能清晰辨識：
![字體比較預覽](https://github.com/max32002/swei-bell-leg/raw/master/preview/compare_style.png)
附註：目前有些筆畫還沒有套用到效果，是程式需要改進的地方。

## 與思源黑體比較
第一行是「獅尾喇腿黑體」，第二行是原版的「思源黑體」：

![字體比較](https://github.com/max32002/swei-bell-leg/raw/master/preview/compare_noto_sans.png)
...
### 「CJK TC」和「CJK SC」的比較：
![TC和SC比較](https://github.com/max32002/swei-bell-leg/raw/master/preview/compare_tc_sc.png)
...
### 「CJK TC」和「CJK JP」的比較：
![TC和JP比較](https://github.com/max32002/swei-bell-leg/raw/master/preview/compare_tc_jp.png)
...
## 更新日誌
[點擊此處](https://github.com/max32002/swei-bell-leg/blob/master/change_log.md) 查看更新記錄。
...
```
@font-face {
  font-family: SweiBellLegCJKtc-Regular;
  src: url(https://cdn.jsdelivr.net/gh/max32002/swei-bell-leg@2.0/WebFont/CJK%20TC/SweiBellLegCJKtc-Regular.woff2) format("woff2")
  , url(https://cdn.jsdelivr.net/gh/max32002/swei-bell-leg@2.0/WebFont/CJK%20TC/SweiBellLegCJKtc-Regular.woff) format("woff");
}
```
